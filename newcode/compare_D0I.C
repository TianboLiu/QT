...
int LoadData(const double QTmax, const char * file){
  ...
  int Npt = 0;
  while (infile >> QT[Npt] >> ds[Npt] >> Estat[Npt]){
    if (QT[Npt] < QTmax){
      flag = 1;
...
double Theory(const double QT){
  double xl[2] = {66.0, -3.3};
  double xu[2] = {116.0, 3.3};
...
int main(const int argc, const char * argv[]){
  ...
  if (argc < 3){
    cout << "./fit_D0I <QT> <fitfile>" << endl;
    return 0;
  }

  TString filename = "path/DY/DY.D0_RunI.list";
