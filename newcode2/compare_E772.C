...
int GetParameters(const char * file){
  ...
  for (int i = 0; i < 8; i++)
    fp.getline(tmp, 300);
  fp >> pars[0][0] >> pars[0][1] >> pars[0][2] >> pars[0][3];
  fp.getline(tmp, 300);
  TMatrixD cov(4,4);
  fp >> cov(0,0) >> cov(0,1) >> cov(0,2) >> cov(0,3);
  fp >> cov(1,0) >> cov(1,1) >> cov(1,2) >> cov(1,3);
  fp >> cov(2,0) >> cov(2,1) >> cov(2,2) >> cov(2,3);
  fp >> cov(3,0) >> cov(3,1) >> cov(3,2) >> cov(3,3);
...
int Curve(){
  ...
  for (int ix = 0; ix < 50; ix++){
    ...
    error = sqrt(pow(theory[1] - theory[2], 2) + pow(theory[3] - theory[4], 2) + pow(theory[5] - theory[6], 2) + pow(theory[7] - theory[8], 2)) / 2.0;
    fprintf(fc, "%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\t%.3E\n",
	    var[1], theory[0], error, theory[1], theory[2], theory[3], theory[4], theory[5], theory[6], theory[7], theory[8]);
  }
  fclose(fc);
  return 0;
}
